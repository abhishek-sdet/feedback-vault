...
async function getDb() {
  ...
  if (connectionString && (connectionString.startsWith('postgres://') || connectionString.startsWith('postgresql://'))) {
    ...
    if (!_pool) {
      ...
      // Force bypass for self-signed certificates in serverless environments
    ...
    try {
      const initialSecret = process.env.DASHBOARD_SECRET || 'Sdet@2026';
      await _db.exec(`CREATE TABLE IF NOT EXISTS Registry (key TEXT PRIMARY KEY, value TEXT)`);
      await _db.prepare(`INSERT INTO Registry (key, value) VALUES ('DASHBOARD_SECRET', ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`).run(initialSecret);
      await _db.exec(`
